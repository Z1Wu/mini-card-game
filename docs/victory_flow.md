...
flowchart TD
    ...
    D2 --> DOUBT[质疑区总和最大且>0 者 存为 imprisoned_player_ids（多人并列则都视为被监禁）]
...
- 若总和最大值 **> 0**，则所有等于该最大值的玩家均视为**被监禁**，存为 `imprisoned_player_ids`（多人并列则都视为被监禁）。
- 供判定 3 使用，**不在此处直接决定胜者**。
...
|--------|--------------|--------------------------|----------|
| 1      | 外星人       | 被监禁即可获胜           | 该玩家 id 是否在 imprisoned_player_ids 中 |
| 2      | 感染者       | 调和失败即可获胜         | 否 harmony_reached |
| 3      | 犯人 / 共犯 / 优等生 | 不被监禁 / 犯人获胜即可获胜 | 该玩家未被监禁；或 犯人方胜（所有被监禁者手牌均无犯人） |
...
所有人只剩一张手牌 → 进入胜败判定
  ...
  → 判定 2：质疑区总和最大且>0 者（多人并列则都视为被监禁）→ 存 imprisoned_player_ids
  → 判定 3：按优先级 1→5 依次
        1：有「被监禁即可获胜」且该玩家在 imprisoned_player_ids？ → 先满足者胜

...
/** Deterministic, transport-free browser fixture used to verify Issue #80 layouts. */
export const GameTableFixture: React.FC = () => {
  ...
  return <main className="campus-shell min-h-screen p-3 sm:p-4"><div className="mx-auto max-w-6xl space-y-4"><header className="campus-panel p-4"><p className="campus-kicker">Issue #80 deterministic fixture</p><h1 className="campus-title text-xl">{count} 人牌桌布局</h1><p role="status" className="mt-2 rounded-lg bg-[#fff4e8] p-2 text-sm font-semibold text-slate-700">▶ {action}</p></header><GameTable players={players} localPlayer={players[0]} localPlayerId="player-0" currentPlayerIndex={0} harmonyArea={[card('harmony-1', CardType.NEWS_CLUB), card('harmony-2', CardType.RICH_GIRL), card('harmony-3', CardType.ACCOMPLICE)]} requiredHarmonyValue={8} isGameOver={false} selectedCard={selectedCard} onSelectCard={setSelectedCard} onPlayCard={handlePlay} newsClubMyChosenCard={null} /></div></main>;
};

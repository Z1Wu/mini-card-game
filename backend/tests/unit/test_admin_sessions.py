...
def test_expired_session_is_invalid():
    mgr = AdminSessionManager(ttl_seconds=60)
    token = mgr.create_session("admin1")
    # Simulate expiry by moving the created timestamp to well before the TTL window
    username, _ = mgr._sessions[token]
    mgr._sessions[token] = (username, time.monotonic() - mgr._ttl - 1)
    assert mgr.validate_session(token) is None
...
def test_cleanup_expired_removes_only_expired():
    ...
    # Force expiry by setting created timestamp to before the TTL window
    username, _ = mgr._sessions[expired_token]
    mgr._sessions[expired_token] = (username, time.monotonic() - mgr._ttl - 1)
